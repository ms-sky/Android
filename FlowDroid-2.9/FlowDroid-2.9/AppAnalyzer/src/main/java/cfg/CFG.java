...
public class CFG {
    ...
    public MultiMap<String,Node> method2Node=new HashMultiMap<>();
    ...
    public void setUseStandardLib(boolean useStandardLib){
        this.isUseStandardLib=useStandardLib;
    }
...
//        为本ICFG指定cg
    ...
    public boolean isMethodInCFG(SootMethod method){
        return method2Node.keySet().contains(method.getSignature());
    }

    protected HeadAndTail buildGraph(SootMethod sootMethod, Set<String> visit, int depth) {
        //我们这样建图，建的
        if (depth > maxDepth)
            return null;
        ...
        if (body != null) {
            ...
            for (Unit unit : unitGraph.getBody().getUnits()) {
                Node node = new Node(unit, sootMethod.getSignature());
                allNodes.put(unit, node);
                method2Node.put(sootMethod.getSignature(),node);
            }
            for (Unit unit : unitGraph.getBody().getUnits()) {
